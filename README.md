...
| 영역 | 기능 |
|---|---|
| 인증/회원 | 회원가입, 이메일 인증, 로그인, JWT 발급, 프로필 수정, 비밀번호 재설정, 관리자 승격 |
| 게시판 | 게시글 CRUD, 검색/필터, 조회수, 댓글/대댓글, 좋아요, 신고 |
| 채용 공고 | 관리자 등록 공고, 외부 공고 수집, 검색/필터, 스크랩 |
| 공모전 | 관리자 등록 공모전, 외부 공모전 수집, 검색/필터, 스크랩 |
| 커뮤니티 | 카테고리 게시판, 일반/익명 글 CRUD, 댓글/대댓글, 좋아요, 신고 |
| 채팅 | 1:1/그룹 채팅방, 메시지 조회/전송, 파일 첨부, 읽음 처리 |
...
이메일 인증/비밀번호 재설정 메일을 사용하려면 SMTP 값을 추가합니다. 설정 상세는 [이메일 인증 가이드](fastapi-vue-board/docs/email-verification.md)를 참고합니다.
...
상세 명세는 [API 명세](docs/04-API-명세.md)를 참고합니다.

## 문서

프로젝트 구조와 설계 분석은 `docs/` 폴더에 정리되어 있습니다.

| 문서 | 내용 |
|---|---|
| [문서 가이드](docs/00-문서-가이드.md) | 문서 사용 방법과 갱신 규칙 |
| [시스템 개요](docs/01-시스템-개요.md) | 전체 구조, 런타임 구성, 주요 도메인 |
| [기능 설계](docs/02-기능-설계.md) | 사용자/관리자/AI/실시간 기능 |
| [데이터베이스 설계](docs/03-데이터베이스-설계.md) | 주요 테이블과 관계 |
| [API 명세](docs/04-API-명세.md) | REST/WebSocket 엔드포인트 |
| [업무 플로우](docs/05-업무-플로우.md) | 회원, 게시글, 신고, 알림 등 흐름 |
| [프론트엔드 구조](docs/06-프론트엔드-구조.md) | Vue 라우팅, 상태, API 호출 구조 |
| [백엔드 구조](docs/07-백엔드-구조.md) | FastAPI 라우터, 서비스, 인증 구조 |
| [운영 배포](docs/08-운영-배포.md) | 로컬 실행, 환경 변수, 배포 점검 |
| [위험요소 및 개선 포인트](docs/09-위험요소-및-개선-포인트.md) | 보안/운영/구조 개선 과제 |
...
- DB 마이그레이션 도구가 없고 앱 시작 시 `create_all()`과 일부 `ALTER TABLE` 보정을 사용합니다. Alembic 도입이 필요합니다.
- 이메일 인증/재설정은 SMTP 설정에 의존하므로 운영 SMTP 미설정 시 회원가입/재설정이 실패할 수 있습니다.
...
자세한 내용은 [위험요소 및 개선 포인트](docs/09-위험요소-및-개선-포인트.md)를 참고합니다.
